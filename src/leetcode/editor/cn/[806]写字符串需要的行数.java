package leetcode.editor.cn;//我们要把给定的字符串 S 从左到右写到每一行上，每一行的最大宽度为100个单位，如果我们在写某个字母的时候会使这行超过了100 个单位，那么我们应该把这个字
...
class NeedRowSolution {
    public int[] numberOfLines(int[] widths, String s) {
        int row = 1,cel = 0;
        int maxLen = 100,rowLen=0;
        for (int i = 0; i < s.toCharArray().length; i++) {
            int width = widths[s.charAt(i)-'a'];
            if(rowLen+width>maxLen){
                row++;
                rowLen=0;
            }
            rowLen+=width;
        }
        return new int[]{row,rowLen};
    }

    public static void main(String[] args) {
        int[] widths = new int[]{10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10};
        NeedRowSolution solution = new NeedRowSolution();
