...
// 
// Related Topics 哈希表 数学 字符串 👍 1857 👎 0
...
class RomanToIntSolution {
    public int romanToInt(String s) {
        ...
        String[] remons = { "M", "CM", "D", "CD", "C", "XC", "L", "XL", "X", "IX", "V", "IV", "I" };
        int[] values = { 1000, 900, 500, 400, 100, 90, 50, 40, 10, 9, 5, 4, 1 };
        int start = 0, end = 1;
        int res = 0;
        for (int i = 0; i < remons.length && start < s.length(); i++) {
            while (end <= s.length() && start - end <= 2) {
                String str = s.substring(start, end);
                if (remons[i].equals(str)) {
                    res += values[i];
                    start = end++;
                } else {
                    end++;
